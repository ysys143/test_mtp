...
echo "[fix2] 31B-bf16-mtp util=0.97 eager maxlen=8192 $(date)"
sudo docker run -d --name gr --gpus all --ipc=host -v ~/.cache/huggingface:/root/.cache/huggingface \
  ...
  vllm/vllm-openai:nightly --model google/gemma-4-31B-it --max-model-len 8192 --gpu-memory-utilization 0.97 \
  --enforce-eager --no-enable-prefix-caching --host 0.0.0.0 --port 8000 --speculative-config "$DR31" >/dev/null 2>&1
HEALTHY=0
for i in $(seq 1 240); do curl -sf http://localhost:8000/health >/dev/null 2>&1 && { HEALTHY=1; break; }
  sudo docker ps -a --filter name=gr --format "{{.Status}}"|grep -qiE "exited|dead" && { echo "[31B-bf16-mtp] DIED-v2"; sudo docker logs gr 2>&1|grep -iE "valueerror|out of memory|estimated max|less than"|tail -3; break; }; sleep 6; done
[ "$HEALTHY" = 1 ] && python3 ~/bench_tp.py "31B-bf16-mtp" || echo "[31B-bf16-mtp] INFEASIBLE-H100-80GB"
